...
function testWithAsync() {
  ...
  async.waterfall([
    ...
    (connection, next) => {
      db = connection;
      db.collection('employees').insertOne({ id: 1, name: '. Promises' }, next);
    },
    (insertResult, next) => {
...
function usage() {
  ...
  console.log('   promises   Use the callbacks paradigm:');
  console.log('   generator   Use the callbacks paradigm:');
  console.log('   async   Use the callbacks paradigm:');
}

if (process.argv.length < 3) {
  console.log('Incorrect number of arugments!');
  usage();
} else {
